...
async def pump_freeswitch_to_openai(
    ...
    # HARD-CODE: This bridge is locked to FreeSWITCH PCM16 mono @ 8000Hz, 20ms frames.
    cfg.fs_channels = 1
    cfg.fs_frame_ms = 20
    cfg.fs_sample_rate = 8000
    ...
    # HARD-CODE: be conservative; OpenAI often rejects commit unless >=100ms is present.
    # We require 200ms worth of appended audio before committing.
    min_commit_ms = 200
    ...
    async for message in upstream_ws:
        if isinstance(message, (bytes, bytearray)):
            if not started:
                ...
                if len(message) != expected_frame_bytes:
                    log(
                        "WARNING: FreeSWITCH sent unexpected frame size; "
                        f"expected {expected_frame_bytes} bytes (8k/20ms/mono) but got {len(message)} bytes. "
                        "Your uuid_audio_stream sampling arg must be 8k/8000 and mix must be mono."
                    )
...
async def pump_openai_to_freeswitch(
    ...
    # If caller left FS_OUT_SAMPLE_RATE unset (0), use cfg.fs_sample_rate (which may be auto-detected).
    fs_out_rate = 8000
    ...
    log(
        "HARD-CODE: Using FreeSWITCH playout format "
        f"{_fmt_audio(fs_out_rate, cfg.fs_channels, cfg.fs_frame_ms)} (frame_bytes={frame_bytes})"
    )
    ...
    # HARD-CODE: keep buffer bounded and realistic for IVR; defaults were too large and can cause huge drops.
    # 2000ms max buffer is plenty.
    max_buf_bytes = _ceil_to_frame(_frame_bytes(8000, 1, max(2000, cfg.fs_frame_ms)), frame_bytes)

    underruns = 0
    # HARD-CODE: 200ms prebuffer for smoother playback.
    prebuffer_bytes = _ceil_to_frame(_frame_bytes(8000, 1, 200), frame_bytes)

    response_in_flight = False
    last_response_create_t = 0.0

    # HARD-CODE: require 200ms of appended audio before committing.
    min_commit_ms = 200
    min_commit_bytes = _frame_bytes(cfg.fs_sample_rate, cfg.fs_channels, min_commit_ms)
    last_turn_evt_t = 0.0
    ...
    async def _maybe_commit(reason: str) -> None:
        ...
        # HARD-CODE: schedule commit slightly later to let the last audio frames arrive.
        nonlocal commit_task
        if commit_task and not commit_task.done():
            return

        async def _delayed_commit() -> None:
            nonlocal commit_pending, commit_reason
        ...
        commit_task = asyncio.create_task(_delayed_commit())
        return
    ...
    try:
        async for message in openai_ws:
            ...
            if is_audio_evt:
                ...
                # HARD-CODE: our internal FS rate is 8k.
                src_rate = 8000
                ...
                # HARD-CODE: already targeting 8k.
                ...
                # For transcription deltas we don't need any extra handling beyond printing.
                # Keep this explicit to avoid future accidental fallthrough changes.
                ...
                if evt_type == "error" and commit_pending:
                    ...
                    if code in ("input_audio_buffer_commit_empty", "input_audio_buffer_commit_too_small"):
                        commit_pending = False
                        commit_reason = None
                        # If server says buffer empty, just keep appending and wait for next turn.
                        # Keep appended_since_commit as-is; we can try committing again once enough builds up.
...
async def main() -> None:
    ...
    # HARD-CODE: FreeSWITCH side is 8k in your dialplan/uuid_audio_stream.
    fs_sample_rate=8000,
        fs_channels=int(os.getenv("FS_CHANNELS", "1")),
    fs_out_sample_rate=8000,
        playout_prebuffer_ms=int(os.getenv("PLAYOUT_PREBUFFER_MS", "300")),
    playout_max_buffer_ms=int(os.getenv("PLAYOUT_MAX_BUFFER_MS", "2000")),
    ...
    log(
        ...
        f"{_fmt_audio(8000, 1, 20)} => frame_bytes={_frame_bytes(8000, 1, 20)} (HARD-CODED)"
    )
