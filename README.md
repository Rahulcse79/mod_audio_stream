...
## Push-back audio test (PCM16 binary)

This repo contains a tiny WebSocket test server that can be used to verify the **push-back** audio path.

### What this test checks

- `mod_audio_stream` connects to your WS server
- the WS server sends **20ms PCM16 @ 8kHz mono** frames (320 bytes each) back to FreeSWITCH
- you confirm in FreeSWITCH logs that WRITE-replace injection ran and injected bytes increased

### Run the WS test server
...
```bash
python3 tools/pushback_ws_server.py --host 127.0.0.1 --port 8765 --seconds 10
```

### Verify from FreeSWITCH logs

During/after the call, you should see:

- `mod_audio_stream pushback: first WRITE tick ...`
- `mod_audio_stream pushback: ... recv=... inject=... underruns=...`

Optional helper:

```bash
python3 tools/verify_pushback_logs.py --log /usr/local/freeswitch/var/log/freeswitch/freeswitch.log
```

### WS-only bidirectional sanity check (no FreeSWITCH)

If you want to verify the WebSocket **binary full-duplex** path first (before involving FreeSWITCH), use echo + the selftest client.

Terminal 1 (server):

```bash
python3 tools/pushback_ws_server.py --host 127.0.0.1 --port 8765 --seconds 0 --mode random --echo
```

Terminal 2 (client selftest):
