                                     ...
                                     char* metadata)
{
    ...
    const char *dir = switch_channel_get_variable(channel, "audio_streaming_direction");
    const char *buf_ms_s = switch_channel_get_variable(channel, "STREAM_BUFFER_SIZE");
    const char *in_pkts_s = NULL;

    void *pUserData = NULL;
    int channels = (flags & SMBF_STEREO) ? 2 : 1;

    /* Dialplan-friendly direction override: in/out/both
       - in:  inject inbound WS audio into call only (WRITE replace)
       - out: capture call audio to WS only (READ)
       - both: do both
    */
    {
        if (!zstr(dir)) {
            if (!strcasecmp(dir, "in")) {
                ...
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                                  "audio_streaming_direction='%s' invalid; using flags as provided\n", dir);
            }
        }
    }
    ...
    {
        /* STREAM_BUFFER_SIZE is ms and should be a multiple of 20ms; used for inbound jitter buffering */
        int in_rtp_packets = 10; /* default 200ms */
        if (!zstr(buf_ms_s)) {
            int buf_ms = atoi(buf_ms_s);
            if (buf_ms < 20) buf_ms = 20;
            if (buf_ms > 2000) buf_ms = 2000;
            /* round down to multiple of 20ms */
            buf_ms = (buf_ms / 20) * 20;
            in_rtp_packets = buf_ms / 20;
            if (in_rtp_packets < 1) in_rtp_packets = 1;
        }
        /* pass this preference to streamer glue via a temporary session variable */
        switch_channel_set_variable_printf(channel, "audio_stream_in_rtp_packets", "%d", in_rtp_packets);
        in_pkts_s = switch_channel_get_variable(channel, "audio_stream_in_rtp_packets");
    }

    {
        /* Emit a single config line that matches dialplan vars (helps debugging) */
        switch_codec_t *wcodec = switch_core_session_get_write_codec(session);
        const char *wname = (wcodec && wcodec->implementation && wcodec->implementation->iananame) ? wcodec->implementation->iananame : "(none)";
        int wsr = (wcodec && wcodec->implementation) ? (int)wcodec->implementation->actual_samples_per_second : 0;
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                          "mod_audio_stream start: ws=%s sampling=%dHz dir=%s STREAM_BUFFER_SIZE=%sms in_pkts=%s read=%s/%d write=%s/%d flags=0x%x\n",
                          wsUri ? wsUri : "(null)",
                          sampling,
                          zstr(dir) ? "(unset)" : dir,
                          zstr(buf_ms_s) ? "(unset)" : buf_ms_s,
                          zstr(in_pkts_s) ? "(unset)" : in_pkts_s,
                          (read_codec && read_codec->implementation && read_codec->implementation->iananame) ? read_codec->implementation->iananame : "(none)",
                          (read_codec && read_codec->implementation) ? (int)read_codec->implementation->actual_samples_per_second : 0,
                          wname,
                          wsr,
                          (unsigned int)flags);
    }
